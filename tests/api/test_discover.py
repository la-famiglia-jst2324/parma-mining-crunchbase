...
from parma_mining.crunchbase.model import DiscoveryRequest, DiscoveryResponse
...
def mock_crunchbase_client(mocker) -> MagicMock:
    """Mocking CrunchbaseClient's discover method."""
    mock = mocker.patch(
...
def test_discover_endpoint_success(
    client: TestClient, mock_crunchbase_client: MagicMock
):
    request_data = [
    ...
    assert mock_crunchbase_client.called
...
def test_discover_non_existing_company(
    ...
    mock_crunchbase_client.return_value = {}
...
def test_discover_endpoint_with_crunchbase_client_error(
    ...
    mock_crunchbase_client.side_effect = Exception("Mocked Exception")
