...
resource "null_resource" "docker_build" {
  ...
  provisioner "local-exec" {
    ...
    command     = "IMG=${var.region}-docker.pkg.dev/${var.project}/parma-registry/parma-mining-crunchbase:${var.env}-$(git rev-parse --short HEAD) && docker build -t $IMG ./../../ && docker push $IMG && echo $IMG > .image.name"
  }
...
resource "google_cloud_run_service" "parma_mining_crunchbase_cloud_run" {
  name     = "parma-mining-crunchbase-${var.env}"
  location = var.region
...
resource "google_cloud_run_service_iam_policy" "noauth" {
  location = google_cloud_run_service.parma_mining_crunchbase_cloud_run.location
  project  = google_cloud_run_service.parma_mining_crunchbase_cloud_run.project
  service  = google_cloud_run_service.parma_mining_crunchbase_cloud_run.name
